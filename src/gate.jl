...
function gate_eval(code::String; _mod::Module=Main, display_code::String=code)
    ...
    try
        ...
        if has_repl && on_interactive
            result = REPL.call_on_backend(() -> Base.invokelatest(_eval_with_capture, expr), backend)
...
function _should_replay_argv()
    ...
    for (i, arg) in enumerate(argv)
        if arg == "-e" && i < length(argv)
            ...
            occursin(".serve(session_id=", code) && return false
            return true
        end
    end
...
function _exec_restart(name::String, session_id::String, project_path::String)
    ...
    args = if _should_replay_argv()
        ...
        # inject our own --project / -i / -e KaimonGate.serve(...).
        ...
        # called KaimonGate.serve() and picked up KAIMON_RESTART_SESSION, the gate
        ...
        $(@__MODULE__).serve(session_id=$(repr(session_id))$ns_kwarg$mirror_kwarg$restart_kwarg$tcp_kwargs)
        """
...
function handle_message(request::NamedTuple)
    ...
    elseif msg_type == :ping
        ...
        _kv = try
            parentmodule(@__MODULE__).PACKAGE_VERSION
        catch
            "unknown"
        end
        return (
